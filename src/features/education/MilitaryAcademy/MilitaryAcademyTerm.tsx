...
import { ChoiceTermReward, SkillTermReward } from "../../CareerDetails/CareerTypes/TermAndRankRewardTypes";
...
export const MilitaryAcademyTerm = () => {
	...
	const [deferredSkills, setDeferredSkills] = useState<(AnyTermReward | string)[]>([])
	const navigate = useNavigate();

	if (!branch) {
		console.error("no branch found in military academy term handler!")
		//TODO: Error boundary here!
		return <p>How did you get here? Hopefully this disappears on its own...</p>
	}
	...
	const handleStart = () => {
		...
		const skillArray = initialArray.filter((e: AnyTermReward) => e.type === 'skill');
		const choiceArray = initialArray.filter((e: AnyTermReward) => e.type === 'choice');
		if (choiceArray.length > 0) {
			setChoices(choiceArray as ChoiceTermReward[]);
			setDeferredSkills(initialArray.filter((e: AnyTermReward) => e.type !== 'choice'));
			setSelectSkill(true);
			return;
		}
	...
	const handleSelect = (event: React.SyntheticEvent<HTMLButtonElement>, index: number) => {
		if (index === 0) {
			setChoices(prev => prev.slice(1));
		} else {
			setChoices(prev => prev.slice(0, index).concat(prev.slice(index + 1)));
		}

...
import { AllSkills, increaseToZero, setValue } from "../../Skills/SkillsSlice";
...
export const UniversityTerm = () => {
    ...
    const minorCleanup = (event: React.FormEvent<HTMLFormElement>) => {
        ...
        if (educationState.majorSpecialty) {
            dispatch(setValue({skill: educationState.major as keyof AllSkills, value: 1, specialty: educationState.majorSpecialty}))
        } else {
            dispatch(setValue({skill: educationState.major as keyof AllSkills, value: 1}));
        }
        dispatch(ageUp());

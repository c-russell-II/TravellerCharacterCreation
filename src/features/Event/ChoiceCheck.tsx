...
import { ChoiceCheckEvent, StatCheckEvent, SkillCheckEvent, anyCheckEvent } from "../CareerDetails/CareerTyping";
import { AllSkills } from "../Skills/SkillsSlice";
import { updateEvent } from "../TermSlice/TermSlice";

const ChoiceCheck = (props: {cleanup: () => void, event: ChoiceCheckEvent}) => {
    ...
    const handleSubmit = (ev: React.FormEvent<HTMLFormElement>) => {
        ...
        if (choice in skills) {
            ...
            const specialtyList = event.specialtyList[choice] === 'any' ? currSkill.specialtiesList : 'any';
            newEvent.checkSkill = choice;
            newEvent.checkType = 'skill';
            let specialty = ''
            let specialtyVal = -3;
        ...
        <>
            ...
            <form name="choice" onSubmit={handleSubmit}>
                {event.choiceList?.map((e: string, i: number) => {
                    return (
                        <label key={'choice label for ' + e + `at ${i}`}><input type="radio" value={e} name="choice" key={`choice radio for ${e} at ${i}`} checked={choice===e} onChange={handleChange}/>{e}</label>
                    )
                })}
...
const eventForming = (event: anyCheckEvent, type: 'skill' | 'stat'): StatCheckEvent | SkillCheckEvent => {

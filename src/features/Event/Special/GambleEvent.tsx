...
const GambleEvent = () => {
    ...
    const career = useSelector((state: RootState)=> state.careers.careerInfo[term.job as string])
    ...
    const runGame = () => {
        setIsReady(false);
        const rollValue = skillCheck((skills[checkSkill as keyof AllSkills] as noSpecSkill).value)
        if (rollValue >= 8) {
            setResult('win')
            return;
        }

...
const CloseHurt = () => {
    ...
    const handleClick = (target: string) => {
        ...
        if (target === 'Parent' || target === 'Sibling') {
            ...
            dispatch(addEvent(`You bring your work home with you, and one of your ${target.toLowerCase()}s is hurt by it - severity: ${severity}`))
            dispatch(resolveEvent());
            return;
        }
        dispatch(addEvent(`One of your contacts or allies is hurt when you bring your work back with you after an assignment. Severity: ${severity}. Description: ${target}`))
        dispatch(resolveEvent());
        return;
    }
    ...
    return (
        <>
            <p>You bring your work home with you, and someone you care about is hurt as a result. Select which contact, ally, or family member is hurt by this.</p>
            {final.map((e, i) => {
                let description = e;
                ...
                return (
                    <div key={i}>
                        <p>{description}</p>
                        <button onClick={() => handleClick(e)}>Select them.</button>
                    </div>
                )
            })}
        </>

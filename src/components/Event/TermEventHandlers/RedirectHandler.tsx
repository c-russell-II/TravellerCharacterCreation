...
import { prisoner } from "../../../Pages/Prison/Utilities/Prisoner";
import { RootState } from "../../../app/store";
import { AnyEvent } from "../../../features/CareerDetails/CareerTyping";

const RedirectHandler = () => {
	const event = useSelector((state: RootState) => state.term.event);
	const job = useSelector((state: RootState) => state.term.job);
	const prevJob = useSelector(
		(state: RootState) => state.careers.previousJob
	);
	...
	const handleRedirect = () => {
		...
		if (!event) {
			console.warn(
				"Error in event handling, redirect handler received null for event from career term!"
			);
			return;
		}
		if (event.type !== "redirect") {
			console.warn(
				"Error in event handling, redirect handler received non-redirect event!"
			);
			return;
		}
		switch (event.destination) {
			case "injury":
				...
				if (!prisoner.specialtiesList.includes(job)) {
					dispatch(
						addDeferredEvents([
							{ type: "special", specialType: "medical" },
						])
					);
				}
				return;
	...
	return (
		<>
			{isDescribedEvent(event as AnyEvent) && (
				<p>{isDescribedEvent(event as AnyEvent)}</p>
			)}
			{body}
		</>
	);
...
const checkCareer = (
	...
	if (typeof currCareer !== "string" && typeof prevCareer !== "string") {
		console.warn(
			"Error in redirect event handler - no valid career saved. Using 'Drifter'"
		);
		return "drifter";
	}
	if (typeof currCareer !== "string") {
		console.warn(
			"Error in redirect event handler - No current career found. Using previous career."
		);

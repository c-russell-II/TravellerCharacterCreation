...
import { AllSkills, AnySkill } from "../../../features/Skills/SkillsSlice";
...
const CheckEvent = (props: { event: anyCheckEvent }) => {
	...
	const handleCheck = () => {
		...
		if (type === "skill") {
			mod = getSkillMod(
				skills[event.checkSkill as keyof AllSkills],
				event.specialty
			);
			if (mod < 0) {
				mod +=
					skills.JackOfAllTrades.value > 0
						? 3 - skills.JackOfAllTrades.value
						: 0;
			}
		}
	...
	const resolveCheck = (result: boolean, event: anyCheckEvent) => {
		...
		const newEvent = { ...contents, description: description };
	...
	return (
		<>
			...
			{isChoice ? (
				<ChoiceCheck
					cleanup={cleanupFunc}
					event={event as ChoiceCheckEvent}
				/>
			) : (
...
const getSkillMod = (
	...
	if (!skill.specialties) {
		if (specialty) {
			console.warn(
				"No specialties for skill passed to check handler! In Check Event, returning -3 as default value"
			);
			return -3;
		} else {
			return skill.value;
	...
	if (!specialty) {
		console.warn(
			"Attempted to run a skill check on a specialty skill without passing specialties! running check at default -3 value."
		);
	}
	return -3;
};

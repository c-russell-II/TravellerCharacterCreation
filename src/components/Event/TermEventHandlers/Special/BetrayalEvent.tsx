...
import {
	addEnemy,
	removeFriend,
} from "../../../../features/Character/miscBonusSlice";
import {
	addDeferredEvents,
	resolveEvent,
...
import { AnyConnectionReward } from "../../../../features/CareerDetails/CareerTypes/EventRewardTypes";
...
const BetrayalEvent = () => {
	...
	const jobDetails = useSelector(
		(state: RootState) => state.term.jobDetails
	) as CareerSpecialty;
	...
	const handleClick = () => {
		...
		if (
			roll === 2 &&
			(jobDetails.parent === "drifter" || jobDetails.parent === "rogue")
		) {
			dispatch(
				addDeferredEvents([
		...
		if (blindside) {
			dispatch(
				addEnemy({
					value: 1,
					description:
						"Someone who worked very hard to end your career, who blindsided you out of the blue during your time as a " +
						job,
				})
			);
		...
		dispatch(
			addEnemy({
				...
				description:
					desc +
					"They went on to betray you, for reasons as yet unknown.",
			})
		);
		dispatch(resolveEvent());
		return;
	...
	return (
		<>
			{blindside ? (
				<>
					<p>
						Someone you've never seen hide nor hair of neatly
						arranges a startling mishap, very likely resulting in
						the loss of your career- and if not, they certainly gave
						it their all.
					</p>
				...
				<>
					<p>
						Someone you trusted betrays you - and tries very hard to
						get you ousted from your career.
					</p>
					<p>
						What you can recall of why you trusted them:{" "}
						{traitor && traitor.description
							? traitor.description
							: "Missing character description. [sorry!]"}
					</p>
